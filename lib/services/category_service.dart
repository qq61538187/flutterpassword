...
class CategoryInfo {
  ...
  Map<String, dynamic> toJson() {
    return {
      'name': name,
      'color': color
          .value, // ignore: deprecated_member_use - Required for JSON serialization
    };
  }
...
class CategoryService extends ChangeNotifier {
  ...
  Future<void> _loadCategories() async {
    try {
      ...
      if (colorsJson != null) {
        ...
        _categoryColors =
            decoded.map((key, value) => MapEntry(key, Color(value as int)));
      } else {
        // 如果没有保存的颜色，使用默认颜色
  ...
  Future<void> _saveCategories() async {
    try {
      ...
      // 保存颜色
      final colorsMap = _categoryColors.map((key, value) => MapEntry(
          key,
          value
              .value)); // ignore: deprecated_member_use - Required for JSON serialization
  ...
  Future<void> updateCategory(String oldCategory, String newCategory,
    ...
    if (nameChanged) {
      ...
      _categoryColors[trimmedNewCategory] = color ??
          oldColor ??
          _defaultCategories['其他'] ??
